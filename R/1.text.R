output$text1intro = renderPrint({ 
  ...
  if(params$p > 0) {
    cat(paste("<p>The vaccination fraction",varfmt("p",value=params$p,prec=0,percent=1),"influences the initial values of",varfmt("Szero = 1 - p - Izero",value=1-sir_init_i-params$p,prec=3),"and",varfmt("Rzero = p.",value=params$p,prec=3),"Throughout the simulation, immunity gained by vaccination and recover is lost at the same rate",varfmt("omega."),"Additionally, at each time step vaccination influences births&mdash;non-vaccinated births add to",varfmt("S(t)"),"at a rate",varfmt("(1 - p)mu"),"and vaccinated births add to",varfmt("R(t)"),"at a rate",varfmt("pmu"),"(see Equations tab). Because S is continually being replenished this way, as well as through loss of immunity, it is possible that ",varfmt("S(t) > Szero.")))
