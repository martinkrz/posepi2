plots1 = function(R0,ip,lp,id,le,p) {
  ...
  plot2 = plot2 + geom_point(data=quarterly %>% filter(quarter%%4 == 0 & time <= period*2),mapping=aes(x=S,y=I),colour=palette["C1"],size=2.5*plot_line_width)
  plot2 = plot2 + geom_point(data=quarterly %>% filter(quarter%%4 > 0  & time <= period*2),mapping=aes(x=S,y=I),colour=palette["C1"],fill="white",size=2.5*plot_line_width,stroke=plot_line_width,shape=21)
